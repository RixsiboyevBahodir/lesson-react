  ...
  return (
    <div className="pt-3">
      <div className="flex items-center justify-between">
        <Link
          to={"/"}
          className="flex items-center gap-2 text-3xl font-semibold font-roboto"
        >
        ...
        <div className="flex items-center max-w-1/2 w-full gap-2">
          <button className="flex items-center bg-[#7f4dff41] px-4 py-2 gap-2 text-[16px] text-[#743ffa] rounded-md cursor-pointer active:bg-[#7f4dff8f] transition-all">
            <LuGalleryVerticalEnd className="text-[19px]" /> Catalog
          </button>
          <div className="w-full relative">
            ...
            />
            <ul
              className={`absolute bg-blue-300 w-full rounded-sm z-10 overflow-y-auto max-h-40 p-3 ${show ? `block` : `hidden`} `}
            >
              {search?.products?.map((items) => (
                <Link
                  key={items.id}
                  to={`/products/${items.id}`}
                  className="hover:bg-blue-600 flex flex-col"
                >
                  {items.title}
                </Link>
              ))}
