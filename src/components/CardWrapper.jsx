      ...
      <div className="grid grid-cols-5 gap-5">
        {[...Array(10)].map((item, index) => (
          <Card
            ...
            style={{ backgroundColor: "#333", width: "244px" }}
          />
        ))}
      </div>
    );
        ...
        {data?.products?.map((card) => (
          ...
          >
            <div className="bg-[#edf1e3] relative">
              ...
              <p className="bg-red-500 p-2 inline rounded-xl text-white absolute top-2 left-2 text-[10px]">
            ...
            <div className="p-2">
              <p className="line-clamp-1 font-bold text-[#743ffa]">
                {card.title}
              </p>
              ...
              <Link
                to={`/products/${card.id}`}
                className="flex items-center w-full bg-[#743ffa] justify-center gap-2 rounded-xl p-2 font-semibold cursor-pointer text-white"
              >
                <TbBasketPlus /> add to cart
              </Link>
            </div>
          </div>
        ))}
